...
def get_busy_dates(unit=None):
    if unit:
        busy_dates_start = UnitSchedule.objects.filter(
            unit=unit,
            start_work__date__gte=timezone.now().date()).dates('start_work', 'day')
        busy_dates_end = UnitSchedule.objects.filter(
            unit=unit,
            end_work__date__gte=timezone.now().date()).dates('end_work', 'day')
...
class Unit(models.Model):
    unit_name = models.CharField(verbose_name='Наименование', unique=True, max_length=100)
    unit_manager = models.ForeignKey(
        UnitUserProfile, related_name='unit_manager', verbose_name='Начальник установки')
...
class UnitReservation(models.Model):
    ...
    tester = models.ForeignKey(UnitUserProfile, verbose_name="Испытатель")
